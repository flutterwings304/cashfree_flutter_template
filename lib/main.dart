...
//Here we will only print the statement to check payment is done or not
    isSuccess = true;
    setState(() {});
...
//If some error occur during payment this will trigger
  void onError(CFErrorResponse errorResponse, String orderId) {
    // printing the error message so that we can show it to user or  checkourselves for testing
    isSuccess = false;
    setState(() {});
...
//Now we will se some parameter like orderID ,environment,payment sessionID after that we wil create the checkout session which will launch through which user can pay.
...
createSessionID(String orderID) async {
  ...
  if (response.statusCode == 200) {
    //If All the details is correct it will return the response and you can get sessionid for checkout
